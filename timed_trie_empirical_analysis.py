...
def generateTraceAndBuildTrie(regex, K, variability_window = 5, local_clock=False, hasFullTrace = False, full_Timed_Trace = ()):
...
def empiral_evaluation_for_len(K=5, variability_window_length=10, TRACE_LENGTH=5000):
    ...
    for i in list(range(10, TRACE_LENGTH, 10)):
        ...
        empirical_result_var_len.append(generateTraceAndBuildTrie(_regex_trace, K, variability_window_length, False, hasFullTrace, full_timed_trace))
...
def invokeEmpiricalEvaluationWithLength(K, VARIABILITY_WINDOW_LENGTH, TRACE_LENGTH=5000):
    ...
    empirical_result_var_len_DF = pd.DataFrame(empirical_result_var_len, columns=["Length", "Alphabet", "Depth", "Time (sec)"])
